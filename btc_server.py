...
HTML_FILE = "/var/lib/5tratumos/apps/publicpool/dashboard/index.html"
...
_cache = {"pool": {}, "workers": {}, "blocks": [], "ledger": {}, "rpc": {}, "btc_price": 0, "time": 0}


def rpc(method, *args):
    cmd = BTC_CLI + [method] + [str(a) for a in args]
    ...
    def api_full(self):
        ...
        for addr, info in addr_groups.items():
            ...
            lb.append({
                ...
                "expected_payout_usd": round(exp * _cache.get("btc_price", 0), 2),
...
def serve():
    ...
    print(f"Pool dashboard on {HOST}:{PORT}")
    srv.serve_forever()
