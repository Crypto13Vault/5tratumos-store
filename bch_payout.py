#!/usr/bin/env python3
"""
Public BCH Pool Payout Daemon
PROP + 25% finder fee, 1% pool fee, 101-conf maturity queue.
Uses separate wallet container + main BCH node for blockchain queries.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("/var/log/publicpool-bch-payout.log")
    ]
)
log = logging.getLogger("payout-bch")
...
def rpc_node(method, *args):
    def fmt(a):
        ...
        return str(a)
    cmd = BCH_NODE_CLI + [method] + [fmt(a) for a in args]
    r = subprocess.run(cmd, capture_output=True, text=True)
    if r.returncode != 0:
        log.warning("Node RPC error %s: %s", method, r.stderr.strip())
        return None
    try:
...
def rpc_wallet(method, *args):
    def fmt(a):
        if isinstance(a, bool): return "true" if a else "false"
        return str(a)
    cmd = BCH_WALLET_CLI + [method] + [fmt(a) for a in args]
    r = subprocess.run(cmd, capture_output=True, text=True)
    if r.returncode != 0:
        log.warning("Wallet RPC error %s: %s", method, r.stderr.strip())
        return None
    try:
        return json.loads(r.stdout)
    except json.JSONDecodeError:
...
def check_new_block(ledger):
    ...
    if reward < 1.0:
        return ledger, balance
    utxos = rpc_wallet("listunspent")
    if not utxos:
        return ledger, balance
    utxos.sort(key=lambda u: u.get("confirmations", 999))
    newest = utxos[-1]
    txid = newest.get("txid", "")
    txinfo = rpc_wallet("gettransaction", txid)
    if not txinfo:
        return ledger, balance
    block_hash = txinfo.get("blockhash", "")
    if not block_hash:
        return ledger, balance
    block_info = rpc_node("getblock", block_hash)
    if not block_info:
        return ledger, balance
...
def execute_payout(pending, chain_height):
    ...
    result = rpc_wallet("sendmany", "", json.dumps(adjusted))
    if result:
        pending["paid"] = True
        pending["txid"] = result
...
def update_pool_status(ledger):
    ...
    balance = rpc_wallet("getbalance") or 0
...
def main():
    log.info("=== Public BCH Pool Payout Daemon ===")
    ...
    while True:
        try:
            ...
            if len(result) == 6:
                ...
                log.info("Queued payout for block %d: %.8f BCH (%d workers, %.8f fee)",
