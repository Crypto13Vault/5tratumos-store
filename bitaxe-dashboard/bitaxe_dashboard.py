...
def _benchmark_worker(ip, **kwargs):
    ...
    step = 0
    try:
        steps_to_estimate = ((max_freq - start_freq) // freq_step + 1) * ((max_volt - start_volt) // volt_step + 1)
        while current_voltage <= max_volt and current_frequency <= max_freq:
            step += 1
            with _benchmark_progress_lock:
                _benchmark_progress[ip].update({"freq": current_frequency, "volt": current_voltage, "step": step, "message": f"Testing {current_frequency}MHz @ {current_voltage}mV...", "progress_pct": min(95, int(step / steps_to_estimate * 100))})
            ...
            for sample in range(total_samples):
                ...
                with _benchmark_progress_lock:
                    _benchmark_progress[ip]["message"] = f"{current_frequency}MHz @ {current_voltage}mV — sample {sample+1}/{total_samples} temp={temp}°C"
                if sample < total_samples - 1:
                    time.sleep(sample_interval)
            if stopped_early:
                with _benchmark_progress_lock:
                    _benchmark_progress[ip]["message"] = f"Thermal stop: {stop_reason}"
            ...
            new_freq = current_frequency + freq_step
            if new_freq > max_freq:
                new_volt = current_voltage + volt_step
                if new_volt > max_volt:
                    print(f"[*] Benchmark {ip}: max freq {max_freq}MHz / max volt {max_volt}mV reached", flush=True)
                    break
                current_voltage = new_volt
                current_frequency = start_freq
                print(f"[*] Benchmark {ip}: freq sweep done, next at {new_volt}mV / {start_freq}MHz", flush=True)
            else:
...
def fetch_bitaxe(ip):
    try:
        ...
        hr1m = hr if (v := gh(data.get("hashRate_1m", 0))) == 0 else v
        return {
            ...
            "network_diff": data.get("networkDifficulty", data.get("stratumDiff", 0)),
            "asics": hr_mon.get("domains", []),
    ...
    .then(r => r.json()).then(s => {
      ...
      if (s.running) {
        div.innerHTML = '<b>Step ' + (s.step||0) + ':</b> ' + (s.message||'') +
          '<br><span style="font-size:.72rem;color:var(--text3)">' + (s.freq||'?') + 'MHz @ ' + (s.volt||'?') + 'mV</span>';
...
function renderKioskHero(m, i, total) {
  const off = m.error;
  const sd = m.best_session_diff || 0;
  const ms = sd >= 1000000000 ? ' gold' : sd >= 100000000 ? ' milestone' : '';
  const tr = m.throttle_flash ? ' throttle' : '';
  const dots = [];
